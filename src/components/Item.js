...
export default class Item extends THREE.Group {
  ...
  init() {
    ...
    this.texture.onUpdate = () => {
      if (
        ...
        if (overridePos)
          this.mesh.scale.set(
            this.texture.size.x * 2,
            this.texture.size.y * 2,
            1
          );
        else this.mesh.scale.set(this.texture.size.x, this.texture.size.y, 1);
    ...
    let align = this.positionId,
      pos = new THREE.Vector2();
    if (align || align === 0) {
      if (align === 0) pos.set(-1 * sceneOptions.xPovit, sceneOptions.yPovit);
      // top left
      else if (align === 7) pos.set(0, sceneOptions.yPovit);
      // top center
      else if (align === 1) pos.set(sceneOptions.xPovit, sceneOptions.yPovit);
      //  top right
      else if (align === 2)
        pos.set(sceneOptions.xPovit, -1 * sceneOptions.yPovit);
      // bottom right
      else if (align === 8) pos.set(0, -1 * sceneOptions.yPovit);
      // bottom center
      else if (align === 3)
        pos.set(-1 * sceneOptions.xPovit, -1 * sceneOptions.yPovit);
      // bottom left
      else if (align === 4) pos.set(-1 * sceneOptions.xPovit, 0);
      // middle left
      else if (align === 5) pos.set(sceneOptions.xPovit, 0);
      // middle Right
  ...
  addVideoButton() {
    ...
    this.videoBox.onClick = () => {
      ...
      if (
        !this.data.videoData.videoId ||
        !this.data.videoData.videoType ||
        !this.data.videoData.mode
      )
        throw new Error("Need to Add VideoData");
      openModal(this.data.videoData);
      //console.log("video ID", this.texture.name);
      //console.log("video Type", "Local");

      if (isRoadBlock) {
        this.mainSpace.c.scrollPos =
          this.zPosition + this.nextStepLocation * -1 + 1100;
  ...
  addImageText() {
    ...
    let fontSize = 24;
    let startX = 0;
    let fontText = "TimesNewRomanPSMT";
    if (
      this.data.type === textType3D.oneText ||
      this.data.type === textType3D.titlewithHeader
    ) {
      fontSize *= 5;
    }
    if (this.data.type === textType3D.quotes) {
      startX = 1 * this.data.text[1].length * 0.5;
      fontText = "HelveticaNeue";
    }
    ...
    for (let i = 0; i < this.data.text.length; i++) {
      ...
      let textGeom = new THREE.TextGeometry(text, {
        font: this.mainSpace.assets.fonts[fontText],
        size: fontSize,
        height: 0,
        curveSegments: 4,
      }).center();
      ...
      } else if (this.data.type === textType3D.quotes && i === 0) {
        fontSize = fontSize * 3;
        startY -= 100; //test height
        startX = 0;
        fontText = "TimesNewRomanPSMT";
      } else {
        if (this.data.type === textType3D.quotes) {
          startY -= 100; //test height
          if (i === 1)
            startX = (this.data.text[2].length - this.data.text[1].length) * 10;
        } else startY -= 45; //test height
      }
    }
  ...
  addImageCap() {
    ...
    if (this.data.copyRight && this.data.copyRight !== "") {
      ...
      this.copyRight = new THREE.Mesh(
        copyRightGeom,
        this.mainSpace.alphaTextMat
      );
    ...
    if (this.data.link !== "") {
      ...
      this.linkGroup.position.set(
        0,
        this.caption
          ? this.caption.position.y - 40
