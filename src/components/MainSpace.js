...
import assetData, { textType3D } from "../configs/assetData";
...
export default class MainSpace {
  ...
  init() {
    ...
    const fov =
      (180 * (2 * Math.atan(this.c.size.h / 2 / cameraPosition))) / Math.PI; // TODO: fix mobile scaling
  ...
  getTargetObjByIndex(targetIndex, step) {
    let targetObjIndex = Object.values(this.assets.textures[step]).findIndex(
      (x) => x.orderId === targetIndex
    );
    let objKey = Object.keys(this.assets.textures[step])[targetObjIndex];
    if (objKey) {
      return objKey;
    } else throw new Error("not obj ffffffffffffff");
  }
  initMainSpace() {
    ...
    this.alphaTextMat = new THREE.MeshBasicMaterial({
      color: 0xffffff,
      transparent: true,
      opacity: 0.5,
      visible: false,
    ...
    this.textOutlineMat = new THREE.MeshBasicMaterial({
      color: steps.intro.outlineTextColor,
      transparent: true,
    });
    ...
    for (let step in this.steps) {
      ...
      if (step !== "intro" && step !== "end") {
        let zPosition = this.sections[step].name.length < 1 ? 400 : 800,
          id;
        let targetIndex = 0;
        for (
          let i = 0;
          i < Object.keys(this.assets.textures[step]).length;
          i++
        ) {
          ...
          if (targetTextureData.type !== TextureTypeEnum.grid) {
            ...
            if (
              this.items[id].data &&
              this.items[id].data.type === textType3D.quotes
            ) {
              this.moreSpace = -300;
            }
            ...
            let stop0 =
              (this.items[Object.keys(this.items)[i - 1]].position.z +
                nextStepLocation -
                gridOptions.startgridOffset) *
              -1;
            this.grids.push({
              items: [],
              stops: [stop0],
            ...
            this.grids[this.grids.length - 1].stops.push(
              (this.items[id].position.z +
                nextStepLocation -
                gridOptions.gridOffset) *
                -1
            );
          }
          zPosition = this.items[id].zPosition + 450 + this.moreSpace;
          targetIndex++;
          this.moreSpace = 0;
        }
      }
  ...
  openTag(item) {
    ...
    if (item.step !== this.activeStep) {
      posOffset = this.sections[
        this.remainingSteps[this.remainingSteps.length - 2]
      ].position.z;
    }
  ...
  closeTag() {
    if (!this.itemAnimating && this.itemOpen) {
      ...
      TweenMax.to(
        [this.captionTextMat, this.linkUnderlineMat, this.alphaTextMat],
        1,
        {
          opacity: 0,
          ease: "Expo.easeInOut",
          onComplete: () => {
            ...
            if (this.itemOpen.linkGroup)
              this.itemOpen.linkGroup.visible = false;
            if (this.itemOpen.copyRight)
              this.itemOpen.copyRight.visible = false;
          },
        }
      );
  ...
  mouseDown(e) {
    ...
    if (this.contactSection.isOpen) {
      ...
      if (this.intersects.length > 0) {
        ...
        if (
          this.intersects[0].object.parent.type === TextureTypeEnum.standard
        ) {
          this.openTag(this.intersects[0].object.parent);
          this.dom.cursor.dataset.cursor = "cross";
        } else if (
          this.intersects[0].object.parent.type === TextureTypeEnum.video
        ) {
          this.intersects[0].object.parent.showVideo();
          this.c.holdingMouseDown = false;
        } else if (this.intersects[0].object.parent.parent) {
          if (
            this.intersects[0].object.parent.parent.type ===
            TextureTypeEnum.roadBlock
          ) {
  ...
  mouseMove(e) {
    if (
      ...
      if (this.intersects.length > 0) {
        if (this.intersects[0].object.parent.uniforms) {
          if (
            this.selectedItem &&
            this.selectedItem.uuid !== this.intersects[0].object.parent.uuid
          ) {
    ...
    if (
      (!this.contactSection.isOpen && this.itemOpen && this.itemOpen.linkBox) ||
      this.activeStep === "end"
    ) {
  ...
  updateOrientation(e) {
    ...
    TweenMax.to(this.camera.rotation, 2, {
      ...
      y: e.gamma
        ? (e.gamma - this.initialOrientation.gamma) * (Math.PI / 300)
        : 0,
      ease: "Power4.easeOut",
    });
  }
  ...
  changeColours(override = false) {
    ...
    if (
      override ||
      (this.remainingSteps[this.remainingSteps.length - 1] &&
      ...
      document.getElementById("fp-nav").children[0].children[
        this.activeIndex
      ...
      if (this.steps[this.activeStep].outlineTextColor) {
        let outlineTextColor = new THREE.Color(
          this.steps[this.activeStep].outlineTextColor
        );
  ...
  animate() {
    ...
    if (this.c.allowScrolling && this.c.scrolling && this.started) {
      ...
      if (
        this.roadBlockIndex < this.roadBlocks.length &&
      ...
      if (
        this.grids.length &&
        this.c.scrollPos >= this.grids[0].stops[0] &&
        this.c.scrollPos <= this.grids[0].stops[1]
      ) {
        if (
          this.c.scrollPos <=
          this.grids[0].stops[0] + gridOptions.startgridStep
        ) {
          //fristpart
          this.grids[0].items.forEach((item) => {
            item.position.z = item.startZ =
              (item.zPosition + Math.random() * 1500) * -1;
          });
  ...
  initListeners() {
    ...
    if (this.loaderEnabled)
      document.getElementById("barTitle").innerHTML =
        "Scroll to explore, <br>click to play video and expand images.";
    ...
    this.tinyGesture = new TinyGesture(this.renderer.domElement, {
      mouseSupport: false,
    });
  ...
  hoverAnim(item, value) {
    ...
    if (value !== 0) {
      ...
      if (this.selectedItem) this.selectedItem.uniforms.progress.value = 0;
    } else this.colorInAction = false;

    TweenMax.to(item.uniforms.progress, 0.65, {
      value: value,
      ease: "Expo.easeInOut",
      onComplete: complete,
    });
